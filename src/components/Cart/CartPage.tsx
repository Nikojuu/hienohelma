"use client";

import Subtitle from "@/components/subtitle";
import { useCart } from "@/hooks/use-cart";
import { Loader2 } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";
import { CampaignAddedCartItems } from "./CampaignAddedCartItems";
import {
  calculateCartWithCampaigns,
  type Campaign,
} from "@putiikkipalvelu/storefront-sdk";

export type ShipmentMethods = {
  id: string;
  name: string;
  min_estimate_delivery_days: number | null;
  max_estimate_delivery_days: number | null;
  cost: number;
};

const CartPage = ({ campaigns }: { campaigns: Campaign[] }) => {
  const [validationError, setValidationError] = useState<string | null>(null);
  const [isValidating, setIsValidating] = useState(false);

  const router = useRouter();
  const cart = useCart();

  const {
    calculatedItems,
    cartTotal,
    originalTotal,
    totalSavings,
    freeShipping,
  } = calculateCartWithCampaigns(cart.items, campaigns);

  const buyXPayYCampaign = campaigns.find(
    (campaign) => campaign.type === "BUY_X_PAY_Y" && campaign.isActive
  );

  useEffect(() => {
    if (validationError) {
      setValidationError(null);
    }
  }, [cart.items]);

  const handleCheckout = async () => {
    if (isValidating) return;
    setIsValidating(true);

    try {
      setValidationError(null);
      const validation = await cart.validateCart();

      if (validation.hasChanges) {
        const messages: string[] = [];
        if (validation.changes.removedItems > 0) {
          messages.push(
            `${validation.changes.removedItems} tuotetta poistettiin (loppu varastosta tai poistettu)`
          );
        }
        if (validation.changes.quantityAdjusted > 0) {
          messages.push(
            `${validation.changes.quantityAdjusted} tuotteen maaraa vahennettiin varastotilanteen mukaan`
          );
        }
        if (validation.changes.priceChanged > 0) {
          messages.push(
            `${validation.changes.priceChanged} tuotteen hinta paivitettiin`
          );
        }

        toast({
          title:
            "Ostoskorissa on tapahtunut muutoksia. Tarkista ostoskori ennen jatkamista.",
          description: messages.join(". "),
          variant: "default",
        });

        setValidationError(
          "Tuotteissa on tapahtunut muutoksia tarkista ostoskori ennen jatkamista"
        );

        return;
      }

      router.push("/payment/checkout");
    } catch (error) {
      console.error("Validation failed:", error);
      toast({
        title: "Virhe",
        description: "Ostoskorin tarkistus epaonnistui. Yrita uudelleen.",
        variant: "destructive",
      });
    } finally {
      setIsValidating(false);
    }
  };

  return (
    <section className="pt-8 md:pt-16 pb-16 bg-soft-ivory">
      <Subtitle subtitle="Ostoskori" />

      <div className="container mx-auto px-4 max-w-screen-xl">
        <div className="lg:grid lg:grid-cols-12 lg:gap-x-12">
          {/* Cart Items Section */}
          <div className="lg:col-span-7">
            <h2 className="sr-only">Ostoskorin tuotteet</h2>

            {cart.loading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="relative p-6 bg-pearl/30 border border-stone/10 animate-pulse"
                  >
                    <div className="flex gap-6">
                      <div className="w-24 h-24 bg-blush/20 rounded" />
                      <div className="flex-1 space-y-3">
                        <div className="h-5 bg-blush/20 rounded w-3/4" />
                        <div className="h-4 bg-blush/20 rounded w-1/2" />
                        <div className="h-4 bg-blush/20 rounded w-1/4" />
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            ) : cart.items.length === 0 ? (
              <div className="relative p-8 md:p-12 bg-pearl/30 border border-stone/10">
                <div className="flex flex-col items-center justify-center text-center">
                  {/* Decorative element */}
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-8 h-[1px] bg-gradient-to-r from-transparent to-blush/40" />
                    <div className="w-2 h-2 rounded-full border border-blush/60" />
                    <div className="w-8 h-[1px] bg-gradient-to-l from-transparent to-blush/40" />
                  </div>

                  <div aria-hidden="true" className="relative mb-6 h-40 w-40">
                    <Image
                      src="https://dsh3gv4ve2.ufs.sh/f/PRCJ5a0N1o4i4qKGOmoWuI5hetYs2UbcZvCKz06lFmBSQgq9"
                      fill
                      loading="eager"
                      alt="Tyhja ostoskori"
                      className="object-contain opacity-80"
                    />
                  </div>

                  <h3 className="font-primary text-4xl text-midnight mb-2">
                    Ostoskorisi on tyhja
                  </h3>
                  <p className="text-base font-secondary text-midnight/60 mb-8">
                    Loyda itsellesi sopiva vaate valikoimastamme
                  </p>

                  <Link
                    href="/products"
                    className="group inline-flex items-center gap-3 px-8 py-4 bg-midnight text-soft-ivory font-secondary text-sm tracking-wider uppercase transition-all duration-300 hover:bg-blush hover:text-midnight"
                  >
                    <span>Selaa tuotteita</span>
                    <svg
                      className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={1.5}
                        d="M17 8l4 4m0 0l-4 4m4-4H3"
                      />
                    </svg>
                  </Link>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <CampaignAddedCartItems
                  buyXPayYCampaign={buyXPayYCampaign}
                  calculatedItems={calculatedItems}
                />
              </div>
            )}
          </div>

          {/* Order Summary Section */}
          <section className="mt-12 lg:mt-0 lg:col-span-5">
            <div className="relative bg-pearl/40 p-6 md:p-8">
              {/* Border frame */}
              <div className="absolute inset-0 border border-stone/15 pointer-events-none" />

              {/* Header */}
              <div className="flex items-center gap-3 mb-6">
                <div className="w-2 h-2 rounded-full border border-blush/60" />
                <h2 className="font-primary text-4xl text-midnight">
                  Tilauksen yhteenveto
                </h2>
              </div>

              <div className="space-y-4">
                {/* Campaign savings */}
                {totalSavings > 0 && (
                  <div className="space-y-3 pb-4 border-b border-stone/15">
                    <div className="flex items-center justify-between">
                      <span className="text-base font-secondary text-midnight/60">
                        Alkuperainen hinta
                      </span>
                      <span className="text-base font-secondary text-midnight/60">
                        {`${(originalTotal / 100).toFixed(2)} €`}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-base font-secondary text-wine">
                        Kampanja saasto
                      </span>
                      <span className="text-base font-secondary text-wine font-medium">
                        {`-${(totalSavings / 100).toFixed(2)} €`}
                      </span>
                    </div>
                  </div>
                )}

                {/* Free shipping status */}
                {freeShipping.campaignName && (
                  <div className="py-4 border-b border-stone/15">
                    <div className="relative p-4 text-center">
                      <div className="absolute inset-0 border border-blush/20 pointer-events-none" />
                      {freeShipping.isEligible ? (
                        <div>
                          <p className="text-base font-secondary text-midnight">
                            <span className="text-blush">✓</span> Ilmainen
                            toimitus!
                          </p>
                          <p className="text-sm font-secondary text-midnight/50 mt-1">
                            {freeShipping.campaignName}
                          </p>
                        </div>
                      ) : (
                        <div>
                          <p className="text-base font-secondary text-midnight/70">
                            Lisaa tuotteita{" "}
                            <span className="text-blush font-medium">
                              {(freeShipping.remainingAmount / 100).toFixed(2)} €
                            </span>{" "}
                            arvosta
                          </p>
                          <p className="text-sm font-secondary text-midnight/50 mt-1">
                            ja saat ilmaisen toimituksen!
                          </p>
                        </div>
                      )}
                    </div>
                  </div>
                )}

                {/* Shipping note */}
                <div className="py-3">
                  <p className="text-sm font-secondary text-midnight/50">
                    {freeShipping.isEligible
                      ? "Ilmainen toimitus sisallytetty!"
                      : "Toimitusmaksu lisataan kun toimitustapa on valittu"}
                  </p>
                </div>

                {/* Total */}
                <div className="flex items-center justify-between pt-4 border-t border-blush/20">
                  <span className="font-secondary text-midnight uppercase tracking-wider text-base">
                    Yhteensa
                  </span>
                  <span className="text-lg text-midnight">
                    {`${(cartTotal / 100).toFixed(2)} €`}
                  </span>
                </div>

                {/* Savings badge */}
                {totalSavings > 0 && (
                  <div className="text-center pt-2">
                    <span className="inline-block text-sm font-secondary text-wine bg-wine/10 px-4 py-2">
                      Saastat {(totalSavings / 100).toFixed(2)} € kampanjalla!
                    </span>
                  </div>
                )}
              </div>

              {/* Checkout button */}
              {cart.items.length > 0 && (
                <div className="mt-8">
                  {/* Validation error banner */}
                  {validationError && (
                    <div
                      className="mb-4 p-4 bg-wine/10 border border-wine/30 text-wine"
                      role="alert"
                      aria-live="assertive"
                      aria-atomic="true"
                    >
                      <p className="text-sm font-secondary">
                        {validationError}
                      </p>
                    </div>
                  )}

                  <button
                    onClick={handleCheckout}
                    disabled={cart.loading || isValidating}
                    className="group w-full inline-flex items-center justify-center gap-3 px-8 py-4 bg-midnight text-soft-ivory font-secondary text-sm tracking-wider uppercase transition-all duration-300 hover:bg-blush hover:text-midnight disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {cart.loading || isValidating ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin" />
                      </>
                    ) : (
                      <>
                        <span>Jatka tilaukseen</span>
                        <svg
                          className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={1.5}
                            d="M17 8l4 4m0 0l-4 4m4-4H3"
                          />
                        </svg>
                      </>
                    )}
                  </button>
                </div>
              )}

              {/* Continue shopping link */}
              <div className="mt-4 text-center">
                <Link
                  href="/products"
                  className="inline-flex items-center gap-2 text-sm font-secondary text-midnight/60 hover:text-blush transition-colors duration-300"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={1.5}
                      d="M7 16l-4-4m0 0l4-4m-4 4h18"
                    />
                  </svg>
                  <span>Jatka ostoksia</span>
                </Link>
              </div>
            </div>
          </section>
        </div>
      </div>
    </section>
  );
};

export default CartPage;
